...
程序每次启动会自动检查一次更新，也可在「设置 → 关于」右侧点「检查更新」。发现新版本时，
更新窗口会先列出该版本的完整更新内容；确认后从 GitHub Releases 下载官方安装包、校验
SHA-256，通过后静默覆盖安装并自动重启。校验不通过会删除下载文件并报错，不会安装未通过
校验的包。
...
- 2560×1440 已完成端到端实机验证；1920×1080 已用 14 个关键页面/状态完成实际
  OCR 与锚点回放（含游戏客户区 1919×1080 的一像素偏差）。其他 16:9 分辨率仍需扩大样本。
...
本项目采用 [MIT 许可证](LICENSE)。你可以使用、复制、修改、合并、发布和分发本软件，
但必须保留原版权声明与许可证文本。第三方组件仍分别适用其上游许可证。
